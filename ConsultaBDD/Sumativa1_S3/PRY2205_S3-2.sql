...
-- Preguntarle al usuario el rango de busqueda:
PROMPT Ingrese renta mínima (ej: 190000):
ACCEPT RENTA_MINIMA NUMBER
PROMPT Ingrese renta máxima (ej: 1200000):
ACCEPT RENTA_MAXIMA NUMBER

SELECT
-- Rut con puntos, guión y DV
-- El upper es por si existiera un rut con 'k' se remplace con 'K'
  REPLACE(TO_CHAR(TO_NUMBER(c.numrut_cli), 'FM999,999,999,999'), ',', '.') || '-' ||
  UPPER(c.dvrut_cli)                                            "RUT Cliente",

-- Nombre Completo, se concatena nombre + apellido paterno apellido materno
  INITCAP(c.nombre_cli || ' ' || c.appaterno_cli || ' ' || c.apmaterno_cli)
                                                              "Nombre Completo Cliente",
 
-- Direccion con manejo de nulos
  INITCAP(NVL(c.direccion_cli, 'Sin Dirección'))                                    "Direccion Cliente",
  
  
-- Renta en CLP, alineado hacia la derecha
  LPAD('$' || REPLACE(TO_CHAR(TRUNC(c.renta_cli), 'FM999,999,999'), ',', '.'), 12, ' ')   "Renta Cliente",

-- Celular en el formato requerido, agregandole los guiones y el 0 inicial
  SUBSTR(LPAD(c.celular_cli, 9, '0'), 1, 2) || '-' ||
  SUBSTR(LPAD(c.celular_cli, 9, '0'), 3, 3) || '-' ||
  SUBSTR(LPAD(c.celular_cli, 9, '0'), 6, 4)                                    "Celular Cliente",
...
-- Tramos pedidos
  CASE
    ...
    ELSE                                                     'TRAMO 4'
  END                                                   "Tramo Renta Cliente"

FROM cliente c
WHERE c.celular_cli IS NOT NULL -- Asegurarse que tomemos los datos que tengan telefono
  AND c.renta_cli BETWEEN &RENTA_MINIMA AND &RENTA_MAXIMA -- La busqueda va entre los rangos que ingresamos
ORDER BY INITCAP(c.nombre_cli || ' ' || c.appaterno_cli || ' ' || c.apmaterno_cli) ASC; -- Ordenado ascendente por nombre
...
-- Preguntar al usuario el promedio minimo del sueldo
PROMPT Ingrese sueldo promedio mínimo (ej: 1100000):
ACCEPT SUELDO_PROMEDIO_MINIMO NUMBER

SELECT
-- codigo categoria
  ce.id_categoria_emp                          "CODIGO_CATEGORIA",

  --Descripción de la categoria + manejo de nulos
  INITCAP(NVL(ce.desc_categoria_emp, 'Sin categoría'))  "DESCRIPCION_CATEGORIA",

  --Cantidad de empleados
  COUNT(*)                                             "CANTIDAD_EMPLEADOS",

  --Código de sucursal: lo dejo comentado porque la tabla no lo tiene como tal.
  --s.id_sucursal                                "CODIGO_SUCURSAL",

  --Descripción de sucursal + manejo de nulos
  INITCAP(NVL(s.desc_sucursal, 'Sin sucursal'))        "SUCURSAL",

  --Sueldo promedio: $ y miles con punto + alineado a la derecha
  LPAD('$' || REPLACE(TO_CHAR(ROUND(AVG(e.sueldo_emp),0), 'FM999,999,999'), ',', '.'),17, ' ')    "SUELDO_PROMEDIO"
...
--Restricción de datos: el sueldo no puede ser nulo o no podrá hacer el calculo
WHERE e.sueldo_emp IS NOT NULL

GROUP BY
  ce.id_categoria_emp, ce.desc_categoria_emp,
  s.id_sucursal,      s.desc_sucursal

--Restricción de grupos: cálculo del promedio 
HAVING AVG(e.sueldo_emp) >= &SUELDO_PROMEDIO_MINIMO

--Orden descendente por sueldo promedio 
...
SELECT
-- codigo tipo de categoria
  LPAD(p.id_tipo_propiedad, 10, ' ')                                "CODIGO_TIPO",
  ...
  -- total propiedades
  COUNT(*)                                                         "TOTAL_PROPIEDADES",
  
  -- Promedio de arriendo (CLP)
  LPAD('$' || REPLACE(TO_CHAR(ROUND(AVG(p.valor_arriendo),0), 'FM999G999G999'), ',', '.'),
       20, ' ')                                                    "PROMEDIO_ARRIENDO",
       
  -- Promedio de superficie (dos decimales y no se usa replace porque queremos las comas)
  LPAD(TO_CHAR(ROUND(AVG(p.superficie),2), 'FM999G999D00'),
       20, ' ')                                                    "PROMEDIO SUPERFICIE",
  
  -- Arriendo promedio por m2 
  LPAD('$' || REPLACE(TO_CHAR(ROUND(AVG(p.valor_arriendo/NULLIF(p.superficie,0)),0),
                              'FM999G999G999'), ',', '.'),
       20, ' ')                                                    "VALOR_ARRIENDO_M2",
...
--Restricción de datos: el valor del arriendo y la superficie en m2 no pueden ser nulos o no podrá hacer el calculo
FROM propiedad p
WHERE p.valor_arriendo IS NOT NULL
  AND p.superficie      IS NOT NULL
GROUP BY
  p.id_tipo_propiedad
...
--Orden descendente por valor de arriendo por m2
